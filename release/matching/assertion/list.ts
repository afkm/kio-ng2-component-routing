...
export const query = <T>( listQuery:ListQuery<T>|T[] ):{(list:any[]):boolean} => {
  ...
  return ( list:any[] ):boolean => {
    return assertions.length === assertions.filter ( (assertion:{(list:any[]):boolean}) => assertion ( list ) === true ).length
  }
}