...
const padRect = ( rectangle:Rect , padding:number ) => {
...
const drawRect = ( context : CanvasRenderingContext2D , rectangle:Rect ) => {
  context.beginPath()
...
const drawCross = ( context : CanvasRenderingContext2D , rectangle:Rect ) => {
  context.beginPath()
  context.moveTo(rectangle.left,rectangle.top)
  context.lineTo(rectangle.right,rectangle.bottom)
...
const drawText = ( context : CanvasRenderingContext2D , options?:any ) => {
  const {
    fontSize=17, //px!
    fontStyle='lighter',
    fontName='Helvetica',
  ...
  const drawLine = ( text:string|string[] ) => {
...
export function renderDataURL ( options:any ) {
  const {
    width=640,
    height=480,
